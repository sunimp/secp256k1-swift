...
extension ASN1 {
    ...
    struct ASN1ObjectIdentifier: ASN1ImplicitlyTaggable {
        ...
        init(asn1Encoded node: ASN1.ASN1Node, withIdentifier identifier: ASN1.ASN1Identifier) throws {
            ...
            // whether it
            // > is the last in the series: bit 8 of the last octet is zero, bit 8 of each preceding octet is one. Bits
            // 7 to 1 of
            // > the octets in the series collectively encode the subidentifier. Conceptually, these groups of bits are
            // concatenated
            // > to form an unsigned binary number whose most significant bit is bit 7 of the first octet and whose
            // least significant
            // > bit is bit 1 of the last octet. The subidentifier shall be encoded in the fewest possible octets[...].
            // >
            // > The number of subidentifiers (N) shall be one less than the number of object identifier components in
            // the object identifier
            // > value being encoded.
            // >
            // > The numerical value of the first subidentifier is derived from the values of the first _two_ object
            ...
            //
            // Yeah, this is a bit bananas, but basically there are only 3 first OID components (0, 1, 2) and there are
            // no more than 39 children
            // of nodes 0 or 1. In my view this is too clever by half, but the ITU.T didn't ask for my opinion when they
            ...
            // Now we need to expand the subcomponents out. This means we need to undo the step above. The first
            // component will be in the range 0..<40
            // when the first oidComponent is 0, 40..<80 when the first oidComponent is 1, and 80+ when the first
            // oidComponent is 2.
        ...
        private static func writeOIDSubidentifier(_ identifier: UInt, into array: inout [UInt8]) {
            // An OID subidentifier is written as an integer over 7-bit bytes, where the last byte has the top bit
            // unset.
...
extension UInt {
    fileprivate init(sevenBitBigEndianBytes bytes: some Collection<UInt8>) throws {

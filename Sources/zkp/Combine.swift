...
extension secp256k1.Signing.PublicKey {
    ...
    public func combine(_ pubkeys: [Self], format: secp256k1.Format = .compressed) throws -> Self {
        ...
        guard
            withUnsafePointersToPubKeys(allPubKeys.map { $0.rawRepresentation }, { ptrsToCombine in
                secp256k1_ec_pubkey_combine(context, &combinedKey, ptrsToCombine, ptrsToCombine.count).boolValue
            }),
            secp256k1_ec_pubkey_serialize(context, &combinedBytes, &pubKeyLen, &combinedKey, format.rawValue).boolValue
        else {
...
extension secp256k1.Signing.PublicKey {
    /// Executes a closure with an array of `UnsafePointer<secp256k1_pubkey>?` for pointer operations on an array of
    /// `secp256k1_pubkey`.
    /// - Parameters:
    ///   - pubKeys: An array of `secp256k1_pubkey` to be converted to `UnsafePointer<secp256k1_pubkey>?`.
    ///   - body: A closure that takes an array of `UnsafePointer<secp256k1_pubkey>?` and returns a result of type
    /// `Result`.
